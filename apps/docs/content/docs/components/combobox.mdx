...
## Submit a form on close

Add `data-sp-submit-on-close` to a trigger to auto-submit the enclosing `<form>` when the combobox closes, but only if selections changed since open. With no value the closest ancestor form is submitted; with a value it's treated as a form id.

This is ideal for filter toolbars — no Apply button required.
