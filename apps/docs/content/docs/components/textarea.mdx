...
Pair with a [Label](/docs/components/label) and wrap in a [Field](/docs/components/field) for accessible form fields. Connect them using matching `for` and `id` attributes so clicking the label focuses the textarea and screen readers announce it correctly.
